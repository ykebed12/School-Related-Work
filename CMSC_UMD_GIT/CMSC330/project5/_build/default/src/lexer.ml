...
(* New regex for project 5 *)
let re_return = Str.regexp "return";;
let re_comma = Str.regexp ",";;

let keywords = ["for";"from";"to";"while";"int";"bool";"printf";"main";"else";"if";"true";"false";"return"];;
    ...
    if pos >= String.length str then
      ...
      (
        ...
        if Str.string_match re_int str pos then
          ...
          (*New part for project 5*)
        else if Str.string_match re_return str pos then
          Tok_Return::(tok (pos + 6) str)
        else if Str.string_match re_comma str pos then
          Tok_Comma::(tok (pos + 1) str)
        else 
