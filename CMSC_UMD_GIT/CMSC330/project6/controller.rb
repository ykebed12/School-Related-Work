...
module Access
	...
	def authorize(user, session)		session == @sessions[user]
	end
...
module User
	...
	def register(user, filename, file, password, confirm)
		...
		@db.execute(query, [Rack::Utils.escape_html(user),Rack::Utils.escape_html(password),filename])
		true
	end
	...
	def update_prefs(user, session, description, token)
		if !authorize(user,session) || token != get_token(user) 
			return false
		end
		query = %{
		UPDATE Users
		...
		@db.execute(query, [Rack::Utils.escape_html(description), user])
		true
	end
end
...
module Posts
	...
	def publish_post(user, session, content, token)
		...
		@db.execute(query, [user,Rack::Utils.escape_html(content),timestamp])
		true
	end
