    ...
    | Char a -> 
      {
        ...
        q0 = count;
        fs = [count+1];
        delta = [(count, Some a, count+1)]
      }
    | Union (a, b) -> 
      let nfa_a = helper a in
      let nfa_b = helper b in
      let next_state = fresh() in
      {
        sigma = union nfa_a.sigma nfa_b.sigma;
        qs = insert (next_state) (insert count (union nfa_a.qs nfa_b.qs));
        q0 = count;
        fs = [next_state];
        delta = (union nfa_a.delta nfa_b.delta) @ [(count, None, nfa_a.q0); (count, None, nfa_b.q0)] @
            (List.fold_left (fun acc h -> insert (h, None, next_state) acc) [] nfa_a.fs)@
            (List.fold_left (fun acc h -> insert (h, None, next_state) acc) [] nfa_b.fs)
      }
    | Concat (a, b) ->
      let nfa_a = helper a in
      let nfa_b = helper b in
      {
        sigma = union nfa_a.sigma nfa_b.sigma;
        qs = union nfa_a.qs nfa_b.qs;
        q0 = nfa_a.q0;
        fs = nfa_b.fs;
        delta = (nfa_a.delta)@(nfa_b.delta)
          @(List.fold_left (fun acc h -> insert (h, None, nfa_b.q0) acc) [] nfa_a.fs)
      }
    | Star a ->
      let nfa_a = helper a in
      {
        ...
        fs = nfa_a.fs;
        delta = nfa_a.delta@
          (List.fold_left (fun acc h -> (union [(h, None, nfa_a.q0);(nfa_a.q0, None,h)] acc)) [] nfa_a.fs)
      } in
    helper regexp ;;
